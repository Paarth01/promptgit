...
def test_continuous_metric_mann_whitney():
    ...
    baseline = VariantSample(
        ...
        n=len(baseline_vals),
        total_events=len(baseline_vals),
    )
    variant = VariantSample(
        variant_id="b",
        label="variant",
...
def test_guardrail_error_rate_spike():
    ...
    bad_variant = VariantSample(
        ...
        n=100,
        error_count=25,
        total_events=100,  # 25% error rate
    )
...
def test_zero_baseline_rate_gives_undefined_lift_not_a_crash():
    """A baseline success rate of exactly 0.0 makes relative lift
    mathematically undefined (division by zero) -- analyze_experiment
    correctly leaves relative_lift_vs_baseline as None in that case, and
    should NOT be treated as 'missing data' vs. a real 0 rate elsewhere.
    Significance and the mean_value comparison must still work regardless
    (that's the actual bug an earlier version had via a truthy-vs-None
    check bug in this function, plus a crash in winner.py's message
    formatting when it later tried to format that None -- see test_winner.py
    for the crash-path regression test)."""
    baseline = VariantSample(
        variant_id="a",
        label="baseline",
        is_baseline=True,
    ...
    variant_result = next(r for r in results if r.label == "variant")
    assert variant_result.is_significant is True
    assert variant_result.mean_value == pytest.approx(50 / 60)
    assert variant_result.relative_lift_vs_baseline is None  # undefined, correctly so
